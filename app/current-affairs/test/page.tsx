...
export default function CurrentAffairsTestPage() {
  ...
  const [adWatched, setAdWatched] = useState(false) // Ad en tawh em
  ...
  useEffect(() => {
    const loadAd = async () => {
      setLoading(true)
      await showInterstitial() // Ad en phawt
      setAdWatched(true)
      setLoading(false)
    }
    loadAd()
  ...
  const handleRestart = async () => {
    await showInterstitial() // Retry apiangin ad
    setCurrentQ(0)
    setSelected(null)
    setShowAnswer(false)
    setScore(0)
        ...
        <div className="max-w-2xl mx-auto pt-20">
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <div className="flex gap-4 justify-center">
              ...
              <Link href="/current-affairs" className="px-6 py-3 bg-gray-600 text-white rounded-lg font-medium active:scale-95">
                Back to Tests
              </Link>
            </div>
          </div>
